...
export default function ConfirmOrder() {
  ...
  const updateConfirm = async (id) => {
    try {
      await axios.put(`http://localhost:8080/api/orders/confirm/${id}`, { confirm: true });
      fetchOrders();
    } catch (error) {
      console.log("Lỗi khi cập nhật Comfirm:", error);
                      ...
                      order.confirm === false && (
                        <tr key={order.id}>
                          ...
                          <td>
                            ...
                            <Button variant="primary" onClick={() => updateConfirm(order.id)}>Confirm</Button>{' '}
