      ...
      <div className='shoppingCart'>
        ...
        <div className='cart-products'>
            ...
            <div className='cart-product' key={item.product.id}>
              ...
              <div className='product-info'>
                ...
                <span className='product-price'>
                  {(item.product.price - (item.product.price * (item.product.discount / 100))) * item.count}$
                </span>
              </div>
              <button
          ...
          {productsInCart.length > 0 &&
            (
              <Link className='btn checkout-btn' to={'/user/checkout'} onClick={onClose}>
                <Button variant="primary" style={{ fontWeight: 'bold', fontSize: '15px' }}>Proceed to checkout</Button>{' '}
              </Link>
            )}
        </div>
      </div>
